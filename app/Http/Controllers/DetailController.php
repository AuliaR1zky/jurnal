...
class DetailController extends Controller
{
  public function report($tgl_awal, $tgl_akhir){
    ...
    ->select('transaksi.id', 'transaksi.id_petugas', 'petugas.nama_petugas', 'tgl_transaksi')
    ->get();

    $data[]=array(); $no=0;
    foreach ($transaksi as $tr){
        $data[$no]['id_transaksi'] = $tr->id;
        $data[$no]['id_petugas'] = $tr->id_petugas;
        $data[$no]['petugas'] = $tr->nama_petugas;
